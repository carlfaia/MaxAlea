...
class alea_cauchypos : public object<alea_cauchypos> {
public:
    MIN_DESCRIPTION {"Generates a positive Cauchy-distributed random variable"};
    ...
    alea_cauchypos(const atoms& args = {}) {
        if (args.size() >= 1) {
...
private:
    ...
    void generate_random() {
        ...
        double temp;
        double g = m_alpha;
        const double pi_half = 3.141592653589793238462643 * 0.5;

        if (g == 0) {
            error("alea-cauchypos : alpha = 0");
            g = 0.00000001;
        }

        double u = std::generate_canonical<double, 32>(m_gen) * pi_half;
